...
export class ShipmentController {
  ...
  private async createShipmentUnlocked(
    ...
    const creationResult = existing ?? await this.shippingService.createShipment({
      ...
      receiverAddressLine1: order.shippingAddress.addressLine1,
      receiverAddressLine2: order.shippingAddress.addressLine2,
      receiverLandmark: order.shippingAddress.landmark,
  ...
  private async createReversePickupUnlocked(
    ...
    const courierName = forwardShipment?.courierName === 'Delhivery'
      ? 'Delhivery'
      : 'BlueDart';
    if (req.qualityCheckItems !== undefined && courierName !== 'Delhivery') {
      throw new HttpErrors.UnprocessableEntity(
        'RVP QC 3.0 is available only when the forward shipment uses Delhivery.',
      );
    }
    ...
    const reverseRes = await this.shippingService.createReversePickup({
      ...
      qualityCheckItems: req.qualityCheckItems,
    }, courierName);
