...
function validate(env, requiredOrigins = []) {
  ...
  if (env.DELHIVERY_WEBHOOK_SECRET && env.DELHIVERY_WEBHOOK_SECRET.length < 32) {
    errors.push('DELHIVERY_WEBHOOK_SECRET must contain at least 32 characters when configured.');
  }
